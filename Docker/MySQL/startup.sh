...
# parameters
MYSQL_ROOT_PWD=${MYSQL_ROOT_PWD:-"mysqlrootpwd"}
MYSQL_USER=${MYSQL_USER:-"root"}
MYSQL_USER_PWD=${MYSQL_USER_PWD:-"lksjdf78498kdfjh"}
MYSQL_USER_DB=${MYSQL_USER_DB:-"radius"}
...
EOF
	...
	echo "[i] Creating FreeRadius schema"
	cat /etc/FreeRadius/schema.sql >> $tfile
	cat /etc/FreeRadius/setup.sql >> $tfile

	echo "[i] Flushing privileges"
